...
**מה שלב 2 לימד אותנו בכוח (מדידה, לא השערה):** template matching + DTW על MFCC הוא
**תלוי-דובר**. הבנק מסונתז מה־TTS שלנו, ולכן הדיוק שדווח תחילה (50/50) היה TTS→STT של אותו
קול — המערכת נבחנה מול עצמה. אותו משפט בגוון קול אחר קיבל **0.000**. מה כן שרד: עוצמה
(1%–100% → 1.000, בזכות CMVN) וגובה צליל (±7 חצאי טון → 1.000) אחרי שהוספנו הרחבת גיוון
לבנק (pitch shift + רעש בשכבת ה־waveform, לא סינתזה חוזרת: 591 תבניות ב־36 שניות). מה לא:
גוון קול זר ורעש חזק (5dB → 0.00). ניסיון לצרף את מנוע הפורמנטים כ"דובר נוסף" **הרס את הכיול** —
המרחק בתוך-תווית (10.1) עלה על המרחק בין-תווית (7.1) וכל הציונים קרסו לאפס; כלומר הגוון
שולט ב־MFCC יותר מהפונמות. המסקנה התכנונית: כיסוי דוברים לא יכול לבוא מסינתזה, רק
מ**הקלטות של המאזין עצמו** — ולכן `enroll_wav` + `/api/enroll` + לוח האימון ב־HUD הם חלק
מהמוצר, לא כלי עזר. נמדד: קול זר, שתי דגימות → ביטויים *שלא נשמרו* קופצים מ־0.000 ל־1.000,
ופקודה מ־0.08 ל־0.77.

**שתי תקלות זמן-ריצה שנמצאו רק כשהשיחה נהייתה hands-free (שתיהן תוקנו ונבדקות):**

1. **זנב מילת ההשכמה נבלע לתוך הפקודה.** הגלאי צריך רק 0.45s של קול כדי לירות, כלומר הוא יורה
   *באמצע* המילה; שארית "ג'רוויס" המשיכה להיכנס לחוצץ והפכה לתחילת הפקודה. נמדד: פקודה של 1.0s
   הגיעה לזיהוי כ־2.76s קול וקיבלה ביטחון 0.286 — מתחת לסף 0.30, כלומר "הוא לא מקשיב" למרות
   שהמיקרופון עבד. התיקון: אחרי יריית השכמה מחכים להפסקה אמיתית (220ms שקט ב־RMS, לא ב־VAD —
   ה־VAD מחשב את רצפת הרעש מתוך מה שמגישים לו, ולכן על פיסה קצרה של דיבור חזק היא רואה שקט),
   עם תקרה של 1.4s כדי ש"ג'רוויס מה השעה" בנשימה אחת לא ייזרק. אחרי התיקון: 1.60s וביטחון 1.00.
2. **ההשתקה העצמית הייתה אילמת.** בזמן ש־JARVIS מדבר המיקרופון מושתק נגד הד, אבל שום אירוע לא
   אמר זאת — וגם לא הייתה דרך לפתוח אותה כשההשמעה באמת נגמרה. עכשיו: אירוע `voice.muted` עם
   הזמן שנשאר (מוגבל לקצב), `voice_unmute` ב־WS ו־REST שה־HUD שולח ברגע שהנגן סיים, וחסימה
   בצד הלקוח כשהנגן עדיין מדבר — כי אומדן השרת הוא ניחוש והדפדפן הוא האמת.
...
| שיחה חופשית | נפתחת ישירות להאזנה (מילת השכמה = בונוס, לא שער) → VAD → תשובה בקול → המשך האזנה · שומר זנב-השכמה (1.0s שהגיע כ־2.76s וציון 0.29 → 1.60s וציון 1.00) · השתקה עצמית מדווחת + `unmute` כשההשמעה נגמרת · האזנה מחדש בשקט · WS בינארי + REST תאום |
| אימון קול | `GET/POST /api/enroll` · לוח HUD (6 משפטים, עצירה אוטומטית) · teach-back: מה שלא זוהה חוזר כ־WAV + כפתור שמאמן ומריץ מיד · `JARVIS_STT_BANK` · מדד ציון השכמה חי ב־HUD |
| היסטוריית צ'אט | `data/transcript.jsonl` · נשמרת לכל תור · נטענת באתחול · `clear_chat` |
| בדיקות | **469 passed / 0 failed** בשמונה מודולים (158 שניות) |
| קצב דיבור | תוקן מ־4.2 ל־9–12 תווים/שנייה: קיפול אוויר מת בתוך יחידה + תקרת משך לפונמה + שומר קצב כולל · תשובה של 91 תווים: 21.5s ← 10.0s |
...
| הזיהוי תלוי-דובר (נמצא במדידה) | המיקרופון נפתח כבר מקשיב (בלי תלות בהשכמה) · אימון קול ב־HUD וב־CLI · teach-back בלחיצה · מדד ציון חי · הרחבת גיוון לבנק (pitch/noise) · הכול מתועד בכנות ב־README |
