...
| שיחה חופשית | מיקרופון פתוח → VAD → השכמה (0.96–1.00) → זיהוי 27ms → תשובה בקול → המשך האזנה · השתקה עצמית · timeout · WS בינארי + REST תאום |
| היסטוריית צ'אט | `data/transcript.jsonl` · נשמרת לכל תור · נטענת באתחול · `clear_chat` |
| בדיקות | **435 passed / 0 failed** בשמונה מודולים (142 שניות) |
