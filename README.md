...
- 🔄 **`run_with_autofix.bat` v2.2** — **עדכון אוטומטי מגיט בכל הפעלה** (`git pull --ff-only` + stash של שינויים מקומיים, רץ מעותק ב־TEMP שלא משתנה תחתיו), משתמש ב־Backend קיים במקום לפתוח עותק שני, ומפעיל Electron בלי DevTools
