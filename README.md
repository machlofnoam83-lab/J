...
![Version](https://img.shields.io/badge/version-2.4.0-00d2ff)
...
## 🪞 v2.4 - חשיבה אמיתית: מהרהרת לפני, ממשיכה אחרי, וחושבת גם עליך (אוגוסט 2026)

v2.3 הראתה *ש*אדיאל חושבת. v2.4 גורם לה **באמת לחשוב** — שלוש שכבות חדשות, הכול מהמודל הביתי:

- 💭 **הרהור פנימי לפני התשובה** — בין סריקת הזיכרון לניסוח, `AdielMind` מייצר משפט-מחשבה מתוך מילותיך (זרע + בקרת איכות bigram ≥0.55) ומזרים אותו כצעד חשיבה חי: `💭 מהרהרת לעצמי: "..."` — רואים אותה חושבת *בקול* לפני שהיא עונה
- 🪞 **הרהור אחרי שיחה** (`reflect_on_exchange`) — ~שנייה אחרי התשובה מגיעה מחשבת-המשך: סקרנות על הנושא שהעלית, המשכיות גנרטיבית מהמודל, או מחשבה **עליך** לפי העובדות שאספה (שם, מיקום, עבודה). עם rate-limit חכם ובלי הרהורים על פעולות טכניות (ווליום/פתיחת אפליקציות)
- 🔗 **שרשרת מחשבות** — ב-35% מהמחשבות הספונטניות, מחשבה אחת מולידה אחריה עוד אחת תוך כמה שניות ("וזה גורם לי לחשוב גם: ...") — בדיוק כמו אצל בני אדם
- 💜 **מחשבות על המשתמש** — גם בלולאה הספונטנית: אדיאל חושבת על מה שהיא יודעת עליך מהשיחות (`user_facts`), עם תוויות חדשות ב-HUD: 💜 מחשבה עליך / 🤔 סקרנות / 🪞 הרהור / 🔗 שרשרת
- 🎚️ ברירת מחדל: מחשבה ראשונה אחרי 45 שניות, ואחת ל-5 דקות (`ADIEL_THOUGHT_MINUTES`)
...
- 🔄 **`run_with_autofix.bat` v2.2.3** — **עדכון אוטומטי בכל הפעלה**: `git pull` אם יש `.git`, אחרת הורדת ZIP מהשרת (בלי למחוק venv/data/node_modules). רץ מעותק ב־TEMP שמקבל את נתיב הפרויקט האמיתי, יודע מי זה קוד הישן מול Backend חי, ומפסיק בנימוס אם הנתיב לא תקין
- 💬 **"מה הולך" תוקן** — היה מסווג בטעות כחיפוש במילון ("לא מצאתי 'הולך'"); נוספו דפוסי שיחה חדשים (`מה המצב`, `מה נהיה`, `ווסאפ`...), מילות שאלה יצאו מה-centroids, ותבניות תשובה לא נאמרות פעמיים ברצף ולא מציגות "0 מילים"
